...
import { buildSystemPrompt, processBrainExtraction, updateMoodState, getOrCreateEpisode, retrieveMemories } from '@/lib/rem-engine'
        ...
        if (persona) {
            ...
            if (personaId) {
                try {
                    ...
                    chatId = await getOrCreateChatSession(adminSupabase, userId, personaId, { customName, workspaceId })
                    if (lastUserMessage && lastUserMessage.role === 'user') {
                        userMessageId = await saveMessage(adminSupabase, chatId, userId, lastUserMessage)
                    }
                } catch (persErr) {
            ...
            if (userText.length > 3 && personaId) {
                try {
                    ...
                    const adminSupabase = (await import('@/lib/supabase/server')).createAdminClient()
                    if (userEmbedding) {
                        memoryBlock = await retrieveMemories(adminSupabase, userEmbedding, personaId, userId, userText)
                    }
                } catch (memErr) {
                    console.error('❌ [ChatEngine] Memory retrieval error:', memErr)
                }
            }

            try {
                const adminSupabase = (await import('@/lib/supabase/server')).createAdminClient()
                const basePrompt = await buildSystemPrompt(adminSupabase, [persona], userId, memoryBlock, false, auth.tenantId, chatId)
                    ...
                    if (chatId && finalContent) {
                        ...
                        Promise.allSettled([
                            (async () => {
                                ...
                                // 1. RegEx Extractions (Facts & Lockets)
                                let cleanContent = finalContent
                                const saveFactRegex = /\[SAVE_FACT:\s*(\w+)\s*\|\s*(.+?)\]/g
                                let factMatch
                                while ((factMatch = saveFactRegex.exec(finalContent)) !== null) {
                                ...
                                let locketMatch
                                while ((locketMatch = saveLocketRegex.exec(finalContent)) !== null) {
                                    const [fullMatch, content] = locketMatch
                                    const locketContent = content.trim()
                                    const locketEmbedding = await generateEmbedding(locketContent)
                                    
                                    await adminSupabase.from('persona_lockets').insert({
                                        persona_id: personaId as string,
                                        content: locketContent,
                                        embedding: locketEmbedding
                                    })
                                    cleanContent = cleanContent.replace(fullMatch, '').trim()
                                }

                                // 2. Mood & Episodic Updates
                                const lastUserMsg = messages[messages.length - 1]
                                const userText = (lastUserMsg && lastUserMsg.role === 'user') ? lastUserMsg.content : ''
                                
                                // Defaulting domain to 'personal' for now, could be dynamic
                                const currentDomain = 'personal'
                                const episodeId = await getOrCreateEpisode(adminSupabase, userId, personaId as string, currentDomain)

                                const responseTokenCount = Math.ceil(cleanContent.length / 4)
                                await updateMoodState(adminSupabase, userId, personaId as string, currentDomain, responseTokenCount, (persona as any)?.config || {})

                                // 3. Save Memories
                                const assistantEmbedding = await generateEmbedding(cleanContent)
                                ...
                                // 4. Brain Extraction (Profile Graph updates)
                                const providerConfig = {
                                    apiEndpoint: (providerInfo as any).apiEndpoint,
                                    defaultModel: (providerInfo as any).defaultModel,
                                    apiKey: providerKey
                                }

                                await processBrainExtraction(
                                    adminSupabase,
                                    userId,
                                    userText,
                                    cleanContent,
