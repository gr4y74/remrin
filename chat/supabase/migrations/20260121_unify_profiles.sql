-- ============================================================================
-- REMRIN MESSENGER PWA - PHASE 1: PROFILE UNIFICATION
-- Migration: 20260121_unify_profiles
...
-- 3. Migrate existing chat profile data to user_profiles
UPDATE public.user_profiles up
SET 
  messenger_settings = jsonb_build_object(
    'away_message', COALESCE(upc.away_message, ''),
    'show_online', COALESCE(upc.show_online, true),
    'sound_enabled', true,
    'notification_enabled', true
  )
FROM public.user_profiles_chat upc
WHERE up.user_id = upc.user_id
AND up.messenger_settings = '{
  "away_message": null,
  "show_online": true,
  "sound_enabled": true,
  "notification_enabled": true
}'::jsonb; -- Only update if not already customized

-- 4. Create or replace view for backward compatibility
-- This allows existing code to continue working while we migrate
CREATE OR REPLACE VIEW user_profiles_chat_view AS
SELECT 
  up.user_id,
  up.username,
  up.display_name,
  ...
  COALESCE((up.messenger_settings->>'show_online')::boolean, true) as show_online,
  'online' as status, -- Will be managed by presence system
  up.created_at,
  false as is_banned,
  false as is_muted
FROM public.user_profiles up;

-- 5. Add index for messenger queries
CREATE INDEX IF NOT EXISTS idx_user_profiles_messenger_settings 
ON public.user_profiles USING GIN (messenger_settings);

-- 6. Update get_buddies_with_status function to use unified profiles
...
-- 7. Create function to broadcast profile updates
