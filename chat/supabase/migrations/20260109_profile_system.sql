-- ============================================================================
-- REMRIN.AI PROFILE SYSTEM - DATABASE SCHEMA
-- Migration: 20260109_profile_system
-- Agent: Alpha (Database Architect)
-- Safe migration using IF NOT EXISTS for idempotency
-- ============================================================================
...
-- 1. USER PROFILES (Extended profile system)
-- Note: Using IF NOT EXISTS to handle case where table was created manually
...
-- 2. ACHIEVEMENTS
-- Badge/achievement definitions with criteria and metadata
...
-- 4. PROFILE ANALYTICS
-- Time-series analytics data for user profiles
...
-- 6. FEATURED CREATIONS
-- User-selected featured personas for profile showcase
CREATE TABLE IF NOT EXISTS featured_creations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
-- 7. SOCIAL LINKS
-- External social media links for user profiles
...
-- Profile Analytics Indexes
CREATE INDEX IF NOT EXISTS idx_profile_analytics_user_date ON profile_analytics(user_id, date DESC);
CREATE INDEX IF NOT EXISTS idx_profile_analytics_metric ON profile_analytics(metric_type);

-- Featured Creations Indexes
CREATE INDEX IF NOT EXISTS idx_featured_creations_user ON featured_creations(user_id, display_order);
CREATE INDEX IF NOT EXISTS idx_featured_creations_persona ON featured_creations(persona_id);

-- Social Links Indexes
CREATE INDEX IF NOT EXISTS idx_social_links_user ON social_links(user_id, display_order);
...
CREATE POLICY "Public profiles are viewable by everyone"
  ...
  USING (privacy_settings->>'profile' = 'public');
...
DROP POLICY IF EXISTS "Public badges are viewable by everyone" ON user_achievements;
DROP POLICY IF EXISTS "Users can view their own achievements" ON user_achievements;
DROP POLICY IF EXISTS "Users can update their own achievement display settings" ON user_achievements;
...
CREATE POLICY "Public badges are viewable by everyone"
  ...
  USING (
    EXISTS (
      ...
      AND privacy_settings->>'badges' = 'public'
    )
  );
...
-- ============================================================================
-- PROFILE_ANALYTICS POLICIES
-- ============================================================================

DROP POLICY IF EXISTS "Public analytics are viewable by everyone" ON profile_analytics;
...
CREATE POLICY "Public analytics are viewable by everyone"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles
      WHERE user_profiles.user_id = profile_analytics.user_id
      AND privacy_settings->>'analytics' = 'public'
    )
  );
...
DROP POLICY IF EXISTS "Users can view their own themes" ON profile_themes;
DROP POLICY IF EXISTS "Users can insert their own themes" ON profile_themes;
DROP POLICY IF EXISTS "Users can update their own themes" ON profile_themes;
DROP POLICY IF EXISTS "Users can delete their own themes" ON profile_themes;
...
-- ============================================================================
-- FEATURED_CREATIONS POLICIES
-- ============================================================================

DROP POLICY IF EXISTS "Public featured creations are viewable by everyone" ON featured_creations;
DROP POLICY IF EXISTS "Users can view their own featured creations" ON featured_creations;
DROP POLICY IF EXISTS "Users can manage their own featured creations" ON featured_creations;

CREATE POLICY "Public featured creations are viewable by everyone"
  ON featured_creations FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles
      WHERE user_profiles.user_id = featured_creations.user_id
      AND privacy_settings->>'profile' = 'public'
    )
  );

CREATE POLICY "Users can view their own featured creations"
  ON featured_creations FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can manage their own featured creations"
  ON featured_creations FOR ALL
  USING (auth.uid() = user_id);
...
CREATE POLICY "Public social links are viewable by everyone"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles
      WHERE user_profiles.user_id = social_links.user_id
      AND privacy_settings->>'profile' = 'public'
    )
  );
...
-- Apply trigger to profile_themes
...
-- TIME-BASED ACHIEVEMENTS (Blue Gradient)
...
-- CONTRIBUTION ACHIEVEMENTS (Green Gradient)
...
('community_helper', 'Community Helper', 'Helped 50+ users', 'heart-handshake', 'linear-gradient(180deg, #81C784 0%, #43A047 100%)', 'contribution', 'rare', '{"type": "help_count", "count": 50}'),

-- SPECIAL RECOGNITION (Purple/Gold Gradient)
...
-- ACTIVITY ACHIEVEMENTS (Orange Gradient)
...
-- ENGAGEMENT ACHIEVEMENTS (Red/Pink Gradient)
('trendsetter', 'Trendsetter', 'Creation reached 10k views', 'fire', 'linear-gradient(180deg, #EF5350 0%, #C62828 100%)', 'engagement', 'epic', '{"type": "creation_views", "count": 10000}'),
...
('viral_creator', 'Viral Creator', 'Creation reached 100k views', 'trending-up', 'linear-gradient(180deg, #F06292 0%, #AD1457 100%)', 'engagement', 'legendary', '{"type": "creation_views", "count": 100000}'),

-- MILESTONE ACHIEVEMENTS (Cyan Gradient)
('10_creations', '10 Creations', 'Created 10 characters', 'check-circle', 'linear-gradient(180deg, #26C6DA 0%, #00ACC1 100%)', 'milestone', 'common', '{"type": "creation_count", "count": 10}'),
...
('night_owl', 'Night Owl', 'Active during late night hours (50+ sessions)', 'moon', 'linear-gradient(180deg, #5C6BC0 0%, #3949AB 100%)', 'activity', 'rare', '{"type": "night_sessions", "count": 50}'),
('early_bird', 'Early Bird', 'Active during early morning hours (50+ sessions)', 'sunrise', 'linear-gradient(180deg, #FFCA28 0%, #FFA000 100%)', 'activity', 'rare', '{"type": "morning_sessions", "count": 50}'),
('conversation_master', 'Conversation Master', 'Reached 10k messages', 'message-circle', 'linear-gradient(180deg, #7E57C2 0%, #5E35B1 100%)', 'engagement', 'legendary', '{"type": "message_count", "count": 10000}'),
('world_builder', 'World Builder', 'Created characters in 10+ different genres', 'globe', 'linear-gradient(180deg, #26A69A 0%, #00897B 100%)', 'special', 'epic', '{"type": "genre_diversity", "count": 10}')
...
COMMENT ON TABLE user_profiles IS 'Extended user profile system with customization and privacy settings';
COMMENT ON TABLE achievements IS 'Achievement/badge definitions with criteria and metadata';
COMMENT ON TABLE user_achievements IS 'Junction table tracking earned achievements per user';
COMMENT ON TABLE profile_analytics IS 'Time-series analytics data for user profiles';
COMMENT ON TABLE profile_themes IS 'User-customizable profile themes';
COMMENT ON TABLE featured_creations IS 'User-selected featured personas for profile showcase';
COMMENT ON TABLE social_links IS 'External social media links for user profiles';
