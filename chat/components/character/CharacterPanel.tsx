...
export const CharacterPanel: FC<CharacterPanelProps> = ({
    ...
    const isOwner = profile?.user_id === (selectedPersona as any).creator_id
            ...
            >
                ...
                <div className="relative flex-1 min-h-[45%] max-h-[55%] w-full overflow-hidden">
                    {selectedPersona.id === MOTHER_OF_SOULS_ID || (selectedPersona as any).video_url ? (
                        ...
                        <Image
                            src={(selectedPersona as any).hero_image_url || selectedPersona.image_url || "/images/rem_hero.webp"}
                    ...
                    {activeTab === "settings" && (
                        <div className="p-4 space-y-4">
                            ...
                            <div className="space-y-3">
                                ...
                                {/* ✨ PERSONALIZE BUTTON - Available to ALL USERS */}
                                <button
                                    onClick={() => setShowPersonalizeModal(true)}
                                    className="w-full relative overflow-hidden flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-rp-foam/10 to-rp-iris/10 border border-rp-foam/20 hover:border-rp-iris/50 transition-all group"
                                >
                                    <div className="flex items-center justify-center size-10 rounded-lg bg-gradient-to-br from-rp-foam/30 to-rp-iris/30 shadow-inner">
                                        <IconSparkles size={20} className="text-rp-foam" />
                                    </div>
                                    <div className="flex-1 text-left">
                                        <div className="flex items-center gap-2">
                                            <span className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-rp-foam to-rp-iris">
                                                Personalize
                                            </span>
                                            <span className="text-[10px] bg-rp-overlay px-1.5 py-0.5 rounded text-rp-muted">NEW</span>
                                        </div>
                                        <div className="text-xs text-rp-muted">Make {selectedPersona?.name || 'them'} know YOU</div>
                                    </div>
