...
export function PostCard({ post, isOwner, onDelete, onEdit }: PostCardProps) {
    ...
    return (
        <div className="bg-rp-surface rounded-xl p-4 border border-rp-highlight-low hover:border-rp-highlight-med transition-all group animate-fadeIn">
            {/* Header */}
            ...
            {post.media_urls && post.media_urls.length > 0 && (
                <div className={`grid gap-2 mb-4 rounded-xl overflow-hidden ${post.media_urls.length === 1 ? 'grid-cols-1' : 'grid-cols-2'
                    ...
                    {post.media_urls.map((url, i) => (
                        <div key={i} className="relative aspect-video bg-rp-overlay group/media overflow-hidden">
                            <Image
                                src={url}
                                alt={`Post media ${i + 1}`}
                                fill
            ...
            {/* Actions */}
            <div className="flex items-center justify-between pt-2 border-t border-rp-highlight-low">
