...
import { useFeed, FeedFilter } from '@/hooks/feed/useFeed';
...
export function FeedContainer({ showSidebar = true }: { showSidebar?: boolean }) {
    const [filter, setFilter] = useState<FeedFilter>('all');
    const { posts, isLoading, hasMore, loadMore, refresh, setPosts } = useFeed({ filter });
        ...
        <div className="max-w-6xl mx-auto px-4 py-6">
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <div className="flex-1 w-full max-w-2xl mx-auto lg:mx-0 space-y-6">
                    {/* Feed Header / Actions */}
                    <div className="flex items-center justify-between bg-rp-base/80 backdrop-blur-md py-2 -mx-4 px-4 border-b border-rp-highlight-low mb-6">
                            ...
                            <div className="space-y-8">
                                <div className="text-center py-20 px-4 bg-rp-surface rounded-3xl border border-rp-highlight-low/50 border-dashed">
                                    ...
                                    <p className="text-rp-muted max-w-sm mx-auto mb-6">
                                        {filter === 'following'
                                            ? "You're not following anyone yet, or they haven't posted. Check out these suggested souls!"
                ...
                {showSidebar && (
                    <aside className="hidden md:block w-80 sticky top-[88px]">
                        <SuggestedUsers />
                    </aside>
                )}
            </div>
