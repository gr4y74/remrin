...
-- Created: 2026-01-09
-- Purpose: Add comprehensive user profile system with badges, analytics, and customization
...
CREATE POLICY "Users can update their own achievement display"
...
CREATE POLICY "Featured creations are viewable by everyone"
...
-- Achievements are viewable by everyone (read-only for users)
...
-- Activity Milestones (Orange gradient)
...
COMMENT ON TABLE user_profiles IS 'User profile information with customization and privacy settings';
COMMENT ON TABLE achievements IS 'Available achievements/badges that users can earn';
COMMENT ON TABLE user_achievements IS 'Junction table tracking which users have earned which achievements';
COMMENT ON TABLE profile_analytics IS 'Analytics data for user profiles (views, time spent, etc.)';
COMMENT ON TABLE profile_themes IS 'Custom theme configurations saved by users';
COMMENT ON TABLE featured_creations IS 'User-selected featured personas to display on profile';
